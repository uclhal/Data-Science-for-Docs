...
```{r, warning=FALSE}
ggplot(data=RCT, aes(x=RCT$age)) + geom_histogram(binwidth=8)
ggplot(data=RCT, aes(x=RCT$id)) + geom_bar()
ggplot(data=RCT, aes(x=RCT$ps12)) + geom_bar()
ggplot(data=RCT, aes(x=RCT$binary_satisfaction)) + geom_bar()
```
...
```{r}
ggplot(RCT, aes(x=fake_age, group=random, fill=random)) + geom_histogram(position="identity", alpha=0.5, binwidth=6) + theme_bw()
```
